...
public class PieChartRenderer extends DataRenderer {
    ...
    public void drawValues(Canvas c) {
        ...
        float offset = Utils.convertDpToPixel(4.f);
        float textSize = mEntryLabelsPaint.getTextSize();
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                if (drawXOutside || drawYOutside) {
                    ...
                    if (transformedAngle % 360.0 >= 90.0 && transformedAngle % 360.0 <= 270.0) {
                        pt2x = Utils.convertDpToPixel(15f);
                        pt2y = pt1y;
                        ...
                        pt2x = mChart.getWidth()-Utils.convertDpToPixel(15f);
                        pt2y = pt1y;
                    ...
                    if (dataSet.getValueLineColor() != ColorTemplate.COLOR_NONE) {
                        ...
                        mValueLinePaint.setStyle(Style.FILL_AND_STROKE);
                        c.drawCircle(pt0x,pt0y,Utils.convertDpToPixel(3f),mValueLinePaint);
                        c.drawLine(pt0x, pt0y, pt1x, pt1y, mValueLinePaint);
                        c.drawLine(pt1x, pt1y, pt2x, pt2y, mValueLinePaint);
                    }
                    labelPtx = pt2x;
                    labelPty = pt2y+textSize;
                    // draw everything, depending on settings
                    if (drawXOutside && drawYOutside) {

                        drawValue(c, formattedValue, pt2x, pt2y-offset, dataSet.getValueTextColor(j));

                        if (j < data.getEntryCount() && entryLabel != null) {
                            drawEntryLabel(c, entryLabel, labelPtx, labelPty);
                        }
