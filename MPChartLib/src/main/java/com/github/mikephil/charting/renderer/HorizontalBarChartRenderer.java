    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        if (mChart.isDrawBarShadowEnabled()) {
            ...
            for (int i = 0, count = Math.min((int)(Math.ceil((float)(dataSet.getEntryCount()) * phaseX)), dataSet.getEntryCount());
                ...
                c.drawRoundRect(mBarShadowRectBuffer,100,100,mShadowPaint);
            }
        }
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            rectF.set(buffer.buffer[j],buffer.buffer[j+1],buffer.buffer[j+2],buffer.buffer[j+3]);
            c.drawRoundRect(rectF,100,100,mRenderPaint);

            if (drawBorder) {
                c.drawRoundRect(rectF,100,100,mBarBorderPaint);
            }
        }
    }
                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < buffer.buffer.length * mAnimator.getPhaseX(); j += 4) {
                        ...
                        if (dataSet.isDrawValuesEnabled()) {
                            drawValue(c,
                                    formattedValue,
                                    width-Utils.convertDpToPixel(15f),
                                    y + halfTextHeight,
                    ...
                    while (index < dataSet.getEntryCount() * mAnimator.getPhaseX()) {
                        ...
                        if (vals == null) {
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                if (dataSet.isDrawValuesEnabled()) {
                                    drawValue(c, formattedValue, width-15, y + halfTextHeight, color);
                                }
